 ...
 */
export const normalizeImageUrl = (imageUrl: string | null | undefined, customFallbackImage?: string): string => {
  if (!imageUrl) return customFallbackImage || '/cars/Image_not_available.webp';
 ...
 */
export const normalizeImageUrls = (images: (string | null | undefined)[], customFallbackImage?: string): string[] => {
  return images
    .filter((img): img is string => !!img)
    .map(img => normalizeImageUrl(img, customFallbackImage));
};
