  ...
  return (
    <RadioGroup className={className} {...props}>
      {options.map((option, index) => {
        ...
        return (
          <div key={option.value} className={cn("flex items-center gap-3 rtl:flex-row-reverse", itemClassName)}>
            <RadioGroupItem value={option.value} id={id} disabled={option.disabled} />
            <Label htmlFor={id} className={cn("cursor-pointer peer-disabled:cursor-not-allowed peer-disabled:opacity-50", labelClassName)}>
              {option.label}
            </Label>
          </div>
        )
...
export type { RadioOption, RadioGroupWithOptionsProps }
