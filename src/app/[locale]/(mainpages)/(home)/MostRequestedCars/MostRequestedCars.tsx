            ...
            <section className="bg-white py-[60px]">
                <section className="container-custom">
                    <header className='flex items-center justify-between mb-4'>
                        <h2 className={`text-2xl font-bold text-gray-900 align-middle leading-[130%] tracking-normal [font-family:var(--font-zain)] ${isRTL ? 'text-right' : 'text-left'}`}>
                            {t('mostRequestedCars')}
                        </h2>
                    </header>
            ...
            <section className="container-custom">
                <header className='flex items-center justify-between'>
                    ...
                    <ProtectedLink href="/cars">
                        <Button variant="outline" size="lg" className='text-sm font-medium text-gray-900 rounded-[10px]' icon={<ArrowIcon className={`w-4 h-4 ${isRTL ? '' : 'rotate-180'}`} />} >{t('viewAll')}</Button>
                    </ProtectedLink>
                </header>
                <div className="relative mt-4" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
                    ...
                    >
                        <div className="relative">
                            ...
                            <div
                                className={`absolute top-0 bottom-0 w-20 pointer-events-none z-10 ${locale === 'en'
                        ...
                        <div className="flex justify-between items-center mt-4">
                            ...
                            <div className={`flex items-center gap-2  ${isRTL ? 'justify-end' : 'justify-start'}`}>
                                {Array.from({ length: Math.min(3, Math.ceil(cars.length / 3)) }).map((_, index) => {
                            ...
                            <div className={`flex items-center gap-2   ${isRTL ? 'flex-row-reverse' : ''}`}>
                                <CarouselPrevious
                                    className="relative left-0 top-0 translate-y-0 h-10 w-10 rounded-[12px] bg-primary hover:bg-primary-hover border-none text-white  "
                                />
                                <CarouselNext
                                    className="relative right-0 top-0 translate-y-0 h-10 w-10 rounded-[12px] bg-white hover:bg-gray-100 border border-[#ECEEF2] text-gray-900  "
                                />
                            </div>
                        </div>
                    </Carousel>
