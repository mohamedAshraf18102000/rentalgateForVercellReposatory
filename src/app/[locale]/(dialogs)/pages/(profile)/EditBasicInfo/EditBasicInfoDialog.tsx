...
export function EditBasicInfoDialog({ onSave, onClose }: EditBasicInfoProps) {
  ...
  const [lastName, setLastName] = React.useState(clientData?.clientName || "");
  ...
  React.useEffect(() => {
    if (clientData?.profileImage && !imagePreview) {
      setImagePreview(getImageUrl(clientData.profileImage));
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      } else if (clientData?.profileImage) {
        // Extract filename from existing image path
        imageFilename = getImageFilename(clientData.profileImage);
      }
