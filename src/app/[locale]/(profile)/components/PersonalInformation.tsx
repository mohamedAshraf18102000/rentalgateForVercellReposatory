...
function InfoRow({ label, value, field, isHighlighted = false, lastRow = false, onEdit }: InfoRowProps) {
                ...
                <div className={`flex items-center gap-4 ${isRTL ? "ml-3" : "mr-3"}`}>
                    ...
                    <div className={`flex-1  `}>
                        {isHighlighted ? (
                            ...
                            <span className="text-[15px] max-sm:text-[12px] font-medium text-[#595959]">{value}</span>
                        )}
                    </div>
                    <ArrowIcon
...
export default function PersonalInformation() {
    ...
    const getClientTypeLabel = (clientType: string | number | null | undefined): string => {
            ...
            openDialog("EditLicenseImage", {
                ...
                onSave: async (imageFilename: string) => {
                    try {
                        ...
                        toast.error(error instanceof Error ? error.message : t("updateError") || "فشل التحديث");
                    }
                },
            });
            return;
            ...
            onSave: async (newValue: string) => {
                try {
                    ...
                    if ((clientData as any).licenseNumber) {
                        updateData.licenseNumber = getImageFilename((clientData as any).licenseNumber);
                    }
